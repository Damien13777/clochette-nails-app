...
- ⚠️ **Coupe `pnpm dev` avant un run E2E** : Next 16 refuse deux `next dev` depuis le même dossier (verrou `.next/dev`), même sur des ports différents. `db:studio` peut rester ouvert (il ne touche pas `.next`). La base reste isolée (:3100 / `clochette_test`), c'est uniquement le serveur dev de Next qui ne tolère pas le doublon.
