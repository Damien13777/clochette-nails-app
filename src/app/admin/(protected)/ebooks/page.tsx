...
export default async function EbookListPage({
  ...
  const [ebooks, counts] = await Promise.all([
    prisma.ebook.findMany({
      ...
      include: {
        _count: {
          select: {
            purchases: { where: { paymentStatus: "PAID" } },
          },
        },
      },
    }),
      ...
      >
        {FILTERS.map((f) => (
          <Link
            key={f.key}
            href={
              f.key === "all" ? "/admin/ebooks" : `/admin/ebooks?status=${f.key}`
            }
            role="tab"
      ...
      {ebooks.length === 0 ? (
        ...
        <ul className="bg-[var(--color-paper)] border border-[var(--color-line)] rounded-[var(--radius-md)] overflow-hidden divide-y divide-[var(--color-line)]">
          {ebooks.map((e) => {
            ...
            const salesCount = e._count.purchases;
            return (
              <li key={e.id}>
                <Link
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                      <span
                        ...
                        style={{ fontFamily: "var(--font-display)" }}
                      >
                        {formatEuros(e.priceCents)}
                      </span>
                      {e.comparePriceCents != null && (
                        <span
                          className="text-[11px] text-[var(--color-ink-500)] line-through"
                          style={{ fontFamily: "var(--font-display)" }}
                        >
                          {formatEuros(e.comparePriceCents)}
                        </span>
                      )}
                      {!e.pdfUrl && (
                        <span
                          className="text-[10px] uppercase tracking-[0.06em] text-[var(--color-danger)] bg-[var(--color-danger)]/10 border border-[var(--color-danger)]/30 px-2 py-0.5 rounded-full"
                          style={{ fontFamily: "var(--font-display)" }}
                        >
                          PDF manquant
                        </span>
...
async function fetchCounts(): Promise<Record<FilterKey, number>> {
  const [all, draft, published, archived] = await Promise.all([
    ...
    prisma.ebook.count({ where: { status: "ARCHIVED" } }),
  ]);
